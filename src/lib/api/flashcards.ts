...
// Simplified: user inputs word + meaning, backend figures out the rest
export async function addCard(word: string, meaning: string): Promise<Flashcard> {
  await delay(500);
  // Mock "backend" auto-detects POS and reading
  const mockPos = ['noun', 'verb', 'adjective', 'particle'] as const;
  const detectedPos = mockPos[Math.floor(Math.random() * mockPos.length)];
  const newCard: Flashcard = {
    id: `card-${Date.now()}`,
    front: word,
    back: meaning,
    reading: word, // backend would generate this
    partOfSpeech: detectedPos,
    deckId: `deck-${detectedPos}`,
    interval: 1,
    easeFactor: 2.5,
    repetitions: 0,
