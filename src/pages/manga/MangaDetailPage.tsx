  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        console.error(err);
        toast.error(translate('manga:detail.loadFailed', 'Không tải được thông tin manga'));
        navigate('/manga');
      } finally {
        setLoading(false);
      }
  ...
  if (!detail) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] gap-3">
        ...
        <Button onClick={() => navigate('/manga')}>{tc('actions.back')}</Button>
      </div>
    );
  }
    ...
    <div className="p-4 md:p-6 max-w-6xl mx-auto animate-fade-in">
      ...
      <div className="mb-4">
        <Button variant="ghost" size="sm" onClick={() => navigate('/manga')}>
          <ArrowLeft className="w-4 h-4 mr-1.5" /> {t('detail.backList')}
        </Button>
      </div>
        ...
        <div className="space-y-3">
          <div className="space-y-1.5">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <Badge variant="outline" className="gap-1">
                ...
                {t('detail.chapterCount', { count: detail.chapters.length })}
              </Badge>
            </div>
          </div>
          ...
          {sortedChapters.length > 0 && (
            <div className="flex flex-wrap gap-2 pt-1">
              ...
              >
                <BookOpen className="w-4 h-4 mr-1.5" /> {t('detail.readFirst')}
              </Button>
              <Button
                size="sm"
                variant="outline"
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-semibold">{t('detail.chapterList')}</h2>
              ...
              >
                    ...
                    <span className="truncate">
                      {ch.title ||
                        (ch.chapter_index != null
                          ? t('detail.chapterTitle', { index: ch.chapter_index })
                          : t('detail.chapterFallback'))}
                    </span>
                    {ch.chapter_index != null && (
                      <span className="text-xs font-normal text-muted-foreground flex-shrink-0">
                        {t('detail.chapterNumber', { index: ch.chapter_index })}
                      </span>
                    )}
                  </CardTitle>
                </CardHeader>
                <CardContent className="pt-0 pb-3 px-3">
                  {ch.date ? (
                    ...
                    <p className="text-xs text-muted-foreground/70 italic">{t('detail.noDate')}</p>
                  )}
                </CardContent>
              </Card>
            ))}
