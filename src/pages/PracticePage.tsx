...
import type { SentencePractice, PracticeResult, SRSRating, PracticeMode } from '@/lib/api/types';
...
export default function PracticePage() {
  ...
  const handleSubmit = async () => {
    ...
    const cost = sentence.creditCost;
    const allowed = await canSpendCredits(cost);
    if (!allowed) {
      setCreditError(`Không đủ credit (cần ${cost}). Hãy mua thêm credit.`);
      return;
    }
    setCreditError('');
    setChecking(true);
  ...
  const modeLabel = mode === 'jp-to-vn' ? 'Nhật → Việt' : 'Việt → Nhật';

  return (
    <div className="p-4 md:p-6 max-w-2xl mx-auto animate-fade-in">
      <div className="mb-4">
        <h2 className="font-display font-bold text-2xl text-foreground mb-1">Luyện câu</h2>
        <p className="text-sm text-muted-foreground">Dịch câu và AI sẽ kiểm tra ngữ pháp. Tốn credit mỗi lần kiểm tra.</p>
      </div>
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <Button
          variant="outline"
          size="sm"
          onClick={() => setMode(mode === 'jp-to-vn' ? 'vn-to-jp' : 'jp-to-vn')}
      ...
      {sentences.length === 0 ? (
        <div className="text-center py-12 text-muted-foreground">
          ...
          <p className="text-sm">Không có câu nào cần ôn.</p>
        </div>
      ) : !result ? (
        <div>
          <div className="text-xs font-mono text-muted-foreground mb-3">
            {currentIdx + 1} / {sentences.length} · {sentence.difficulty} · {sentence.creditCost} credit
          </div>
          ...
          <Textarea
            ...
            placeholder={mode === 'jp-to-vn' ? 'Nhập bản dịch tiếng Việt...' : 'Nhập bản dịch tiếng Nhật...'}
            className="bg-card border-border mb-3 min-h-[80px]"
          />
          <Button onClick={handleSubmit} disabled={checking || !answer.trim()} className="w-full">
        ...
        <div className="space-y-4">
          ...
          <div className="bg-card border border-border rounded-lg p-4 space-y-2">
            ...
            <div>
              ...
              <p className="text-sm text-foreground font-medium">{result.correctAnswer}</p>
            </div>
          </div>

          {result.grammarNotes.length > 0 && (
            <div className="bg-muted/50 rounded-lg p-4">
              <p className="text-xs font-semibold text-foreground mb-2">Ghi chú ngữ pháp:</p>
