...
import { transcribeVideo, generateClozeIndices, getDefaultClozeSettings, getDefaultVideoPlayerSettings } from '@/lib/api/transcription';
import type { TranscriptSegment, TokenTimestamp, ClozeSettings, ClozeMode, VideoPlayerSettings } from '@/lib/api/transcription';
import { lookupWord } from '@/lib/api/flashcard';
import { Button } from '@/components/ui/button';
import { Slider } from '@/components/ui/slider';
import { ArrowLeft, Eye, EyeOff, Repeat, Play, Pause, SkipBack, SkipForward, Settings, Search, Plus, BookOpen, ChevronLeft, ChevronRight, X } from 'lucide-react';
...
export default function TranscribeViewPage() {
  ...
  useEffect(() => {
    if (!videoId) return;
    setLoading(true);
    transcribeVideo(videoId, true, 0, 0).then(result => {
      if (result.transcript) {
        setSegments(result.transcript.segments);
      }
      setLoading(false);
    });
  }, [videoId]);

  // Regenerate cloze when settings/segments/time change
  useEffect(() => {
  ...
  // Restart interval when playback rate changes
  useEffect(() => {
    if (isPlaying) {
      if (timerRef.current) clearInterval(timerRef.current);
      timerRef.current = setInterval(() => {
  ...
  const isTokenActive = (word: TokenTimestamp) => currentTime >= word.start && currentTime < word.end + 0.15;

  const handleLookup = async (token: string) => {
    setLookupLoading(true);
    const result = await lookupWord(token);
    ...
    <div className="flex h-[calc(100vh-3rem)] md:h-screen animate-fade-in">
        ...
        <div className="p-3 border-t border-border space-y-2">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-1">
              <Button variant="ghost" size="sm" onClick={seekBackward} className="w-8 h-8 p-0">
                <SkipBack className="w-4 h-4" />
              </Button>
              <Button variant={isPlaying ? 'default' : 'outline'} size="sm" onClick={() => isPlaying ? pause() : play()} className="w-8 h-8 p-0">
                {isPlaying ? <Pause className="w-4 h-4" /> : <Play className="w-4 h-4" />}
              </Button>
              <Button variant="ghost" size="sm" onClick={seekForward} className="w-8 h-8 p-0">
            ...
            <div className="flex items-center gap-2">
              ...
              <select
                ...
                onChange={e => setPlayerSettings(prev => ({ ...prev, playbackRate: parseFloat(e.target.value) }))}
              ...
              <select
                value={playerSettings.seekDuration}
                onChange={e => setPlayerSettings(prev => ({ ...prev, seekDuration: parseInt(e.target.value) }))}
                className="text-xs bg-muted border border-border rounded px-1.5 py-0.5 text-foreground"
              >
                {[3, 5, 10, 15, 30].map(s => (
                  <option key={s} value={s}>Tua {s}s</option>
                ))}
              </select>
            </div>
          </div>
      ...
      <div className={`flex flex-col border-l border-border bg-card ${settingsOpen ? 'w-96' : 'w-80'} flex-shrink-0 transition-all`}>
        ...
        <div className="p-2 border-b border-border flex items-center gap-1 flex-wrap">
          ...
          >
            {clozeMode ? <EyeOff className="w-3 h-3" /> : <Eye className="w-3 h-3" />}
            Cloze
          </Button>
          {loopSegmentIdx !== null && (
        ...
        {settingsOpen ? (
          <div className="flex-1 overflow-y-auto p-3 space-y-4">
            <div className="flex items-center justify-between">
              ...
              <Button variant="ghost" size="sm" onClick={() => setSettingsOpen(false)} className="w-6 h-6 p-0">
                <X className="w-3.5 h-3.5" />
              </Button>
            </div>

            {/* Cloze Mode Selection */}
            <div>
              ...
              <div className="space-y-1.5">
                {(Object.keys(CLOZE_MODE_INFO) as ClozeMode[]).map(mode => (
                  <button
                    key={mode}
                    onClick={() => setClozeSettings(prev => ({ ...prev, mode }))}
                    className={`w-full text-left p-2.5 rounded-xl text-xs transition-colors ${clozeSettings.mode === mode ? 'bg-primary/10 border border-primary/30 text-primary' : 'bg-muted text-muted-foreground hover:bg-muted/80'}`}
                  >
              ...
              <div className="space-y-3">
                ...
                {clozeSettings.mode === 'reading' && (
                  <div>
                    ...
                    <Slider
                      value={[clozeSettings.windowOffset]}
                      min={-5}
                      max={5}
                      step={0.5}
                      onValueChange={([v]) => setClozeSettings(prev => ({ ...prev, windowOffset: v }))}
                    />
            ...
            {/* Video settings */}
            <div>
              <p className="text-xs font-bold text-muted-foreground mb-2">Video</p>
              <div className="space-y-2">
                ...
                <div className="flex items-center justify-between">
                  <span className="text-xs text-muted-foreground">Thời lượng tua</span>
                  <select
                    value={playerSettings.seekDuration}
                    onChange={e => setPlayerSettings(prev => ({ ...prev, seekDuration: parseInt(e.target.value) }))}
                    className="text-xs bg-muted border border-border rounded px-2 py-1 text-foreground"
          ...
          <div className="flex-1 overflow-y-auto p-3 space-y-3">
            ...
            {lookupResult && (
              <div className="bg-muted/50 rounded-xl p-3 text-xs space-y-1 relative">
                <Button variant="ghost" size="sm" onClick={() => setLookupResult(null)} className="absolute top-1 right-1 w-5 h-5 p-0">
                  <X className="w-3 h-3" />
                </Button>
            ...
            {segments.map((seg, si) => (
              <div
                ...
                className={`p-3 rounded-xl border transition-all cursor-pointer ${
                  loopSegmentIdx === si ? 'border-primary bg-primary/5' : 'border-border hover:border-primary/30'
                }`}
                onClick={() => setLoopSegmentIdx(loopSegmentIdx === si ? null : si)}
              >
                <div className="flex items-center gap-2 mb-1.5">
                  <span className="text-[10px] font-mono text-muted-foreground">
                    {formatTime(seg.words[0]?.start ?? 0)}
                  </span>
                  {loopSegmentIdx === si && (
                    ...
                    return (
                      <span
                        ...
                        onClick={e => {
                          e.stopPropagation();
                          jumpTo(word.start);
                          handleLookup(word.token.replace(/[。、！？]/g, ''));
                        }}
                      >
                        {word.token}
                      </span>
